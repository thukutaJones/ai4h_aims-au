"""Contains an AST-based math expression resolver for omegaconf.
...
def ast_eval(expression: str):
    ...
    if not all(node_types_supported):
        unsupported_types = ", ".join(
            [str(type(subnodes[nidx])) for nidx, supported in enumerate(node_types_supported) if not supported]
        )
        raise ValueError(
